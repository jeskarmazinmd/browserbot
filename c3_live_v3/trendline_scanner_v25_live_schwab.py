...
def get_schwab_client():
    # C3 clone: borrow the research bot's current MARKET access token over Fly's
    # private network.  The clone never refreshes or writes the market token.
    lease_url = os.environ.get("MARKET_TOKEN_LEASE_URL", "").strip()
    if lease_url:
        from leased_schwab_market_client import LeasedSchwabMarketClient
        return LeasedSchwabMarketClient(
            lease_url=lease_url,
            lease_secret=os.environ.get("MARKET_TOKEN_LEASE_SECRET", ""),
        )
