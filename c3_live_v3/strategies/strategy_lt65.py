"""Standalone prospective paper experiment: Independent L-style exhaustion entry with target price >= $6.50."""
...
STRATEGY_ID = "LT65"
DESCRIPTION = "Independent L-style exhaustion entry with target price >= $6.50"
FAMILY = "LX"
PAPER_ONLY = True
...
def accepts_flash(event: Mapping[str, Any], max_flash_drop_pct: float) -> bool:
    ...
    if not (CONFIG["flash_drop_pct"] <= drop <= float(max_flash_drop_pct)):
        return False
    if _num(event.get('flash_volume_ratio'), -1e99) < 1.0:
        return False
    return _num(event.get('target_price'), -1e99) >= 6.5
...
def validate_confirmed_entry(event: Mapping[str, Any], min_remaining_upside_pct: float) -> tuple[bool, str | None]:
    ...
    flash_ratio = _num(event.get('flash_volume_ratio'), -1.0)
    rebound_ratio = _num(event.get('rebound_volume_ratio'), 1e99)
    if flash_ratio < 1.0 or rebound_ratio > flash_ratio * 0.75:
        return False, 'rebound_volume_not_exhausted'
    timestamp = _timestamp(event)
