"""Standalone prospective paper experiment: Independent P-style midday entry."""
...
STRATEGY_ID = "PMID"
DESCRIPTION = "Independent P-style midday entry"
FAMILY = "PX"
PAPER_ONLY = True
...
def accepts_flash(event: Mapping[str, Any], max_flash_drop_pct: float) -> bool:
    ...
    if not (CONFIG["flash_drop_pct"] <= drop <= float(max_flash_drop_pct)):
        return False
    return _num(event.get('pre_return_pct'), -1e99) >= 0.75 and _num(event.get('pre_r2'), -1e99) >= 0.5
