"""Standalone prospective paper experiment: Independent H-style entry with target price >= $5."""
...
STRATEGY_ID = "HT5"
DESCRIPTION = "Independent H-style entry with target price >= $5"
FAMILY = "HX"
PAPER_ONLY = True
...
CONFIG = {
    ...
    "live_order_placement": False, "min_remaining_upside_pct": 0.1,
}
...
def accepts_flash(event: Mapping[str, Any], max_flash_drop_pct: float) -> bool:
    ...
    if not (CONFIG["flash_drop_pct"] <= drop <= float(max_flash_drop_pct)):
        return False
    if drop > min(2.50, float(max_flash_drop_pct)):
        return False
    if _num(event.get('pre_r2'), -1e99) < 0.40:
        return False
    if _num(event.get('pre_slope_pct_per_hour'), 1e99) > 12.0:
        return False
    return _num(event.get('target_price'), -1e99) >= 5.0
