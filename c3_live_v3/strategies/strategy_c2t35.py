"""Standalone prospective paper experiment: Independent C2-style entry with target price <= $3.50."""
...
STRATEGY_ID = "C2T35"
DESCRIPTION = "Independent C2-style entry with target price <= $3.50"
FAMILY = "C2X"
PAPER_ONLY = True
...
def accepts_flash(event: Mapping[str, Any], max_flash_drop_pct: float) -> bool:
    ...
    if not (CONFIG["flash_drop_pct"] <= drop <= float(max_flash_drop_pct)):
        return False
    return _num(event.get('target_price'), 1e99) <= 3.5
...
def refresh_event_for_entry(event: Mapping[str, Any], current_price: float) -> dict[str, Any]:
    ...
    refreshed.update({'exit_model': 'c2', 'activation_gain_pct': 0.3, 'no_new_high_seconds': 30.0})
    return refreshed
