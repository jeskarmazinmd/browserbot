...
class EMA1Strategy(EventStrategy):
    ...
    def on_snapshot(
        ...
        signals = []; reset(self)
        scalar_volume_provider = snapshot.metadata.get(
            "confirm_recent_volume_ratio"
        )
        batch_volume_provider = snapshot.metadata.get(
        ...
        for symbol, quote in snapshot.quotes.items():
            ...
            consider(self, symbol, snapshot.timestamp, price, [boolean("bullish_ema_crossover", crossed), boolean("volume_provider_available", volume_provider_available)])
