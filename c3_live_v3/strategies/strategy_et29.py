"""Standalone prospective paper experiment: Independent E-style entry with target price <= $29."""
...
STRATEGY_ID = "ET29"
DESCRIPTION = "Independent E-style entry with target price <= $29"
FAMILY = "EX"
PAPER_ONLY = True
...
def accepts_flash(event: Mapping[str, Any], max_flash_drop_pct: float) -> bool:
    ...
    if not (CONFIG["flash_drop_pct"] <= drop <= float(max_flash_drop_pct)):
        return False
    if event.get('volume_data_status_flash') != 'OK':
        return False
    if _num(event.get('flash_dollar_volume_3m')) < 1_200_000:
        return False
    return _num(event.get('target_price'), 1e99) <= 29.0
