"""Standalone prospective paper experiment: Independent Q-style midday volatility-normalized entry."""
...
STRATEGY_ID = "QMID"
DESCRIPTION = "Independent Q-style midday volatility-normalized entry"
FAMILY = "QX"
PAPER_ONLY = True
...
def accepts_flash(event: Mapping[str, Any], max_flash_drop_pct: float) -> bool:
    ...
    if not (CONFIG["flash_drop_pct"] <= drop <= float(max_flash_drop_pct)):
        return False
    std = _num(event.get('pre30_return_std_pct'))
...
def refresh_event_for_entry(event: Mapping[str, Any], current_price: float) -> dict[str, Any]:
    ...
    std = _num(refreshed.get('pre30_return_std_pct'))
    refreshed['flash_drop_volatility_units'] = original_drop_pct / std if std > 0 else 0.0
    return refreshed
...
def validate_confirmed_entry(event: Mapping[str, Any], min_remaining_upside_pct: float) -> tuple[bool, str | None]:
    ...
    if minute is None or not (720 <= minute < 840):
