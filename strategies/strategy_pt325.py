"""Standalone prospective paper experiment: Independent P-style entry with target price >= $3.25."""
...
STRATEGY_ID = "PT325"
DESCRIPTION = "Independent P-style entry with target price >= $3.25"
FAMILY = "PX"
PAPER_ONLY = True
...
def accepts_flash(event: Mapping[str, Any], max_flash_drop_pct: float) -> bool:
    ...
    if _num(event.get('pre_r2'), -1e99) < 0.5:
        return False
    return _num(event.get('target_price'), -1e99) >= 3.25
