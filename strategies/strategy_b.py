...
Strategy B uses the same qualifying flash as Strategy A, but requires a 0.20%
rebound confirmation and uses a 2% protective stop. It is paper-only.

This module owns Strategy B's configuration and entry-specific calculations.
...
STRATEGY_ID = "B"
DESCRIPTION = "1.0% flash drop, 0.20% rebound confirmation, 2% stop"
...
def accepts_flash(event: Mapping[str, Any], max_flash_drop_pct: float) -> bool:
    """Return whether a detected flash satisfies Strategy B's drop range."""
...
def refresh_event_for_entry(
    ...
    """Build Strategy B's rebound-confirmed entry snapshot."""
...
def validate_confirmed_entry(
    ...
    """Validate a rebound-confirmed Strategy B entry."""
