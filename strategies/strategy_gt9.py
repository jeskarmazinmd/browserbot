"""Standalone prospective paper experiment: Independent G-style entry with target price <= $9."""
...
STRATEGY_ID = "GT9"
DESCRIPTION = "Independent G-style entry with target price <= $9"
FAMILY = "GX"
PAPER_ONLY = True
...
def accepts_flash(event: Mapping[str, Any], max_flash_drop_pct: float) -> bool:
    ...
    if not (CONFIG["flash_drop_pct"] <= drop <= float(max_flash_drop_pct)):
        return False
    return _num(event.get('target_price'), 1e99) <= 9.0
...
def refresh_event_for_entry(event: Mapping[str, Any], current_price: float) -> dict[str, Any]:
    ...
    refreshed.update({'exit_model': 'c4', 'activation_gain_pct': 0.3, 'slope_window_seconds': 30.0, 'negative_slope_pct_per_minute': -0.2})
    return refreshed
