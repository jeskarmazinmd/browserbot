...
INPUT_FILE = Path.home() / "Desktop" / "1Mvolumesymbols.csv"
CACHE_DIR = Path.home() / "Desktop" / "trendline_cache"
INTRADAY_CACHE_DIR = Path.home() / "Desktop" / "trendline_intraday_cache"
...
def find_trades_for_symbol(symbol, intraday_close, metrics):
    ...
    for day, day_prices in s.groupby(s.index.date):
        ...
        for i in range(needed - 1, len(day_prices) - 1):
            ...
            if not (pass_pre_slope and pass_pre_return and pass_flash):
                continue
...
def main():
    ...
    output_file = OUTPUT_DIR / f"intraday_replay_{timestamp}.csv"
