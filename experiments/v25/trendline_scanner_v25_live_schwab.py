...
OUTPUT_DIR = Path("/app")
TOKEN_PATH = Path("/app/schwab_token.json")
...
def cache_is_fresh(symbol, today=None):
    ...
    if close is None or len(close) < 60:
        return False
    try:
        last_date = pd.Timestamp(close.index.max()).tz_localize(None).normalize()
    except Exception:
        return False
...
def save_daily_close_cache(symbol, close):
    # Accept Series, single-column DataFrame, or accidentally 2D yfinance output.
    if close is None:
        return False
    ...
    if isinstance(close, pd.DataFrame):
        if close.empty:
            return False
        # Prefer a Close column if present; otherwise take first usable column.
    ...
    # If still somehow 2D/object-ish, flatten defensively.
    if getattr(close, "ndim", 1) != 1:
        close = pd.Series(np.ravel(close))
...
def refresh_daily_cache_batch(symbols, today=None, batch_size=75):
    ...
    for i, batch in enumerate(chunk_list(stale, batch_size), start=1):
        ...
        for symbol in batch:
            old = load_daily_close(symbol)
            new_close = get_series(data, symbol, "Close")
            if new_close is None or new_close.empty:
                # if totally missing, use 6mo bootstrap fallback for just this symbol
                try:
                    single = yf.download(symbol, period="6mo", interval="1d", progress=False, timeout=10)
                    new_close = single["Close"] if single is not None and not single.empty and "Close" in single.columns else None
                except Exception:
                    new_close = None

            if new_close is None or len(new_close) == 0:
                mark_bad_symbols([symbol], reason="no Yahoo daily data")
                continue

            combined = new_close if old is None else pd.concat([old, new_close])
            combined = combined[~combined.index.duplicated(keep="last")].sort_index().tail(150)

            if save_daily_close_cache(symbol, combined):
                refreshed += 1
    ...
    for batch in chunk_list(symbols, QUOTE_BATCH_SIZE):
        try:
            ...
            if resp.status_code != 200:
                print(f"  Schwab quote batch status: {resp.status_code}")
                continue
            data = resp.json()
        except Exception as e:
            print(f"  Schwab quote batch failed: {e}")
            continue
...
def main():
    ...
    for cycle in range(1, LIVE_MAX_CYCLES + 1):
        ...
        for symbol, px in prices.items():
            ...
            event = detect_live_flash_from_quotes(symbol, list(history[symbol]), features_by_symbol.get(symbol, {}))
            if event:
                alerts.append(event)
