"""MSBIDPULL1: abrupt displayed bid-size withdrawal with downside confirmation."""
from collections import defaultdict, deque
from datetime import datetime, timezone
import statistics

STRATEGY_ID="MSBIDPULL1";FAMILY="MICROSTRUCTURE10";PAPER_ONLY=True;LIVE_ORDER_PLACEMENT=False
...
class Strategy:
    ...
    def evaluate(self,snapshot):
        ...
        for s,q in snapshot.get("quotes",{}).items():
            b=float(q["bid"]);a=float(q["ask"]);self.h[s].append(((a+b)/2,float(q.get("bid_size") or 0)))
        ...
        for s,q in snapshot.get("quotes",{}).items():
            ...
            if ratio<=.30 and move<=-.025:c.append(((1-ratio)+abs(move),s,q,{"bid_size_vs_recent_median":ratio,"mid_move_15s_pct":move}))
        if not c:return []
        _,s,q,m=max(c);self.last=now
        return [{"strategy_id":STRATEGY_ID,"timestamp":now.isoformat(),"symbol":s,"side":"SHORT","bid":float(q["bid"]),"ask":float(q["ask"]),"target_pct":.30,"stop_pct":.22,"max_hold_minutes":25,"research":m,"paper_only":True,"live_order_placement":False}]
