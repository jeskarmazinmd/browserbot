...
def curves(quotes):
 ...
 for root in ROOT_MONTHS:
  rows=[q for q in quotes.values() if q["symbol"].startswith(root)];rows.sort(key=lambda q:float(q["expiration_ms"]));out[root]=rows[:3]
 return out
def _atomic(payload):
