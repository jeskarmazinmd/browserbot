...
class MicrostructurePaperTracker:
    """Bid/ask-crossing paper ledger. It contains no strategy calculations."""

    def __init__(self, root="/data", notional=1000.0):
        self.root = Path(root)
        self.notional = float(notional)
        self.ledger = self.root / "microstructure_paper_outcomes.jsonl"
        self.status_path = self.root / "microstructure_paper_status.json"
        self.active = {}
        self.seen = set()
        self.completed = 0
        self._restore()
    ...
    def _status(self):
        _atomic(self.status_path, {
            ...
            "pricing": "LONG open@ask close@bid; SHORT open@bid close@ask; whole shares",
    ...
    def open_decisions(self, decisions):
        ...
        for d in decisions:
            ...
            shares = int(self.notional / entry)
            if shares < 1:
                continue
            ...
            if side == "LONG":
                ...
                stop = entry * (1 + stop_pct / 100)
            row = {
                "event": "OPEN", "setup_id": key, "strategy_id": d["strategy_id"],
                "symbol": symbol, "side": side, "opened_at": now.isoformat(),
                "entry_price": entry, "entry_bid": bid, "entry_ask": ask,
                "shares": shares, "notional_used": shares * entry,
    ...
    def update(self, timestamp, quotes):
        ...
        for key, row in list(self.active.items()):
            ...
            reason = None
            if side == "LONG":
                if exit_price >= row["target_price"]:
                    reason = "TARGET"
                elif exit_price <= row["stop_price"]:
                    reason = "STOP"
            else:
                if exit_price <= row["target_price"]:
                    reason = "TARGET"
            ...
            if reason is None and eod:
                reason = "EOD"
            if reason is None and held >= row["max_hold_minutes"]:
                reason = "TIMEOUT"
            if reason:
                closing.append((key, row, exit_price, bid, ask, reason))
        for key, row, exit_price, bid, ask, reason in closing:
            signed = 1 if row["side"] == "LONG" else -1
            pnl = signed * (exit_price - row["entry_price"]) * row["shares"]
            ret = signed * (exit_price - row["entry_price"]) / row["entry_price"] * 100
            out = {
                "event": "CLOSE", "setup_id": key, "strategy_id": row["strategy_id"],
                "symbol": row["symbol"], "side": row["side"],
                "opened_at": row["opened_at"], "closed_at": now.isoformat(),
                "entry_price": row["entry_price"], "exit_price": exit_price,
                "exit_bid": bid, "exit_ask": ask, "shares": row["shares"],
                "exit_reason": reason, "return_pct": ret, "pnl": pnl,
                "paper_only": True, "broker_execution_enabled": False,
            }
            self._append(out)
            self.active.pop(key, None)
