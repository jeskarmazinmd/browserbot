...
TRIGGER_OUTCOMES_JSONL = Path("/data/trigger_outcomes.jsonl")
...
def save_trigger_outcomes(outcomes):
    try:
        tmp = TRIGGER_OUTCOMES_JSONL.with_suffix(".tmp")
        with tmp.open("w") as f:
            for r in outcomes.values():
                f.write(json.dumps(r) + "\n")
        tmp.replace(TRIGGER_OUTCOMES_JSONL)
    except Exception:
        pass


def trigger_outcome_tracker_lines(trigger_events, max_items=20):
    try:
        import pandas as pd
    except Exception as e:
        return [f"unavailable: pandas import failed: {type(e).__name__}: {e}"]

    outcomes = load_trigger_outcomes()

    # Ensure every trigger has a durable record, even if tape is later trimmed.
    for e in trigger_events:
        try:
            ...
            sym = e.get("symbol") or deep_find(e, "symbol")
            entry = deep_find(e, {"entry", "entry_price", "buy_limit", "price"})
            target = deep_find(e, {"target", "target_price"})
            stop = deep_find(e, {"stop", "stop_price"})
    ...
    # Update outcomes from any tape data still available.
    for k, rec in list(outcomes.items()):
        try:
            ts = str(rec.get("timestamp", ""))
            sym = rec.get("symbol")
            entry = rec.get("entry")
            if not ts or not sym or not entry:
                continue

            day = ts[:10]
            tape_path = Path("/data/tapes") / f"quotes_{day.replace('-', '')}.csv"

            if day not in tape_cache:
                if not tape_path.exists():
                    tape_cache[day] = None
                else:
                    # Stream the CSV in bounded chunks and retain only symbols
                    # that have tracked outcomes.  Loading every quote for all
                    # ~684 symbols every 30 seconds was the largest steady RSS
                    # consumer and could overlap with collector trimming.
                    parts = []
                    wanted = wanted_by_day.get(day, set())
                    ...
                    df = df.rename(columns={"timestamp_utc": "timestamp", "last_price": "price"})
            ...
            if trigger_time.tzinfo is None:
                ...
                trigger_time = trigger_time.tz_convert("UTC")

            sdf = df[(df["symbol"].astype(str) == str(sym)) & (df["timestamp"] >= trigger_time)].sort_values("timestamp")
            if sdf.empty:
                continue

            latest = float(sdf.iloc[-1]["price"])
            max_px = float(sdf["price"].max())
            min_px = float(sdf["price"].min())

            rec["latest_price"] = latest
            rec["max_price"] = max(rec["max_price"], max_px) if rec.get("max_price") is not None else max_px
            rec["min_price"] = min(rec["min_price"], min_px) if rec.get("min_price") is not None else min_px

            target = rec.get("target")
            stop = rec.get("stop")

            if target is not None and max_px >= target:
                rec["target_hit"] = True
            if stop is not None and min_px <= stop:
                rec["stop_hit"] = True

            if rec.get("first_hit", "none") == "none":
                for _, row in sdf.iterrows():
                    px = float(row["price"])
                    if target is not None and px >= target:
                        rec["first_hit"] = "target"
                        break
            ...
            def ret_at(minutes):
                cutoff = trigger_time + pd.Timedelta(minutes=minutes)
                rows = sdf[sdf["timestamp"] >= cutoff]
                if rows.empty:
                    return rec.get(f"ret_{minutes}m")
                px = float(rows.iloc[0]["price"])
                return ((px / float(entry)) - 1) * 100

            rec["ret_5m"] = ret_at(5)
            rec["ret_15m"] = ret_at(15)
            rec["ret_30m"] = ret_at(30)
            rec["ret_60m"] = ret_at(60)
    ...
    recent = sorted(outcomes.values(), key=lambda r: str(r.get("timestamp", "")))[-max_items:]
    if not recent:
        return ["None"]
    ...
    for r in recent:
        try:
            ...
            max_px = r.get("max_price")
            min_px = r.get("min_price")

            def pct(v):
                return "NA" if v is None else f"{float(v):.2f}%"

            if entry and max_px is not None:
                mfe = ((float(max_px) / float(entry)) - 1) * 100
            else:
                mfe = None
            if entry and min_px is not None:
                mae = ((float(min_px) / float(entry)) - 1) * 100
            else:
                mae = None

            lines.append(
                f"{r.get('timestamp')} | {r.get('symbol')} | "
                f"entry={entry if entry is not None else 'NA'} "
                f"latest={latest if latest is not None else 'NA'} "
                f"max={max_px if max_px is not None else 'NA'} "
                f"min={min_px if min_px is not None else 'NA'} "
                f"MFE={pct(mfe)} MAE={pct(mae)} "
                f"target_hit={r.get('target_hit')} stop_hit={r.get('stop_hit')} first={r.get('first_hit')} "
                f"+5m={pct(r.get('ret_5m'))} +15m={pct(r.get('ret_15m'))} "
                f"+30m={pct(r.get('ret_30m'))} +60m={pct(r.get('ret_60m'))}"
            )
        except Exception as e:
            lines.append(f"{r.get('timestamp')} | {r.get('symbol')} | render_error={type(e).__name__}: {e}")
...
def near_miss_paper_lines(top_events, max_items=25):
    ...
    for day, wanted in wanted_by_day.items():
        ...
        try:
            ...
            df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce", utc=True)
            df["price"] = pd.to_numeric(df["price"], errors="coerce")
            tape_cache[day] = df.dropna(subset=["timestamp", "symbol", "price"])
        except Exception:
            tape_cache[day] = None
...
def main():
    ...
    while True:
        try:
            ...
            lines += ["", "TRIGGER OUTCOME TRACKER"]
            for line in trigger_outcome_tracker_lines([e for e in events if e.get("event_type") == "ENTRY_TRIGGER_OCO_ATTEMPT"], max_items=20):
                lines.append(line)
