...
class MarketTokenReliabilityTests(unittest.TestCase):
    ...
    def test_logs_bounded_durable_evidence_without_tokens(self):
        ...
        self.assertIn('record_auth_event("trading_refresh_ok"',self.source);self.assertIn('record_auth_event("trading_refresh_error"',self.source)
    ...
    def test_trading_has_same_persistence_and_verification_standard(self):
        ...
        self.assertIn('DATA_ROOT / "trading_auth_status.json"',block);self.assertIn("after_min_left < TOKEN_REFRESH_VERIFY_MINUTES",block);self.assertIn("post-refresh trading account verification failed",block);self.assertIn('account_verification="enabled"',block)
