...
def trim_tape_file(path):
    try:
        ...
        # Keep header + recent rows, but enforce both row and size limits.
        lines = path.read_text().splitlines()
        if not lines:
            return

        header = lines[0] if lines[0].startswith("timestamp_utc,") else "timestamp_utc,symbol,last_price"
        body = [ln for ln in lines if not ln.startswith("timestamp_utc,")]
        tail = body[-MAX_TAPE_ROWS:]

        while len("\n".join([header] + tail).encode()) > max_bytes and len(tail) > 1000:
            tail = tail[len(tail)//4:]

        path.write_text("\n".join([header] + tail) + "\n")
        print(f"trimmed tape {path} to {path.stat().st_size/1024/1024:.1f}MB rows={len(tail)}", flush=True)
    except Exception as e:
